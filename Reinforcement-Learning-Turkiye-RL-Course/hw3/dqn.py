...
from plot_every_n_step import plot
from backup import *

OptimizerSpec = namedtuple("OptimizerSpec", ["constructor", "kwargs", "lr_schedule"])

backup_data = Backup() #save train data
...
class QLearner(object):
    ...
    """
        ...
        # arrays for plotting the mean reward and best mean reward of each n episode
        self.best_mean_rews = []
        self.mean_rews = []
        self.steps = []
        ...
        self.obs_t_ph = tf.compat.v1.placeholder(
            tf.float32 if lander else tf.uint8, [None] + list(input_shape))
        # placeholder for current action
        self.act_t_ph = tf.compat.v1.placeholder(tf.int32, [None])
        # placeholder for current reward
        self.rew_t_ph = tf.compat.v1.placeholder(tf.float32, [None])
        # placeholder for next observation (or state)
        self.obs_tp1_ph = tf.compat.v1.placeholder(
            tf.float32 if lander else tf.uint8, [None] + list(input_shape))
        ...
        self.done_mask_ph = tf.compat.v1.placeholder(tf.float32, [None])
        ...
        self.q_network = q_func(obs_t_float, self.num_actions, scope='q_func', reuse=False)
        q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='q_func')
        ...
        if not double_q:
            q_t_plus = tf.reduce_max(self.target_q, axis=1)
        else:
            q_temp = q_func(obs_tp1_float, self.num_actions, scope='q_func', reuse=True)
            act_temp = tf.argmax(q_temp, axis=1)
            q_t_plus = tf.reduce_sum(tf.one_hot(act_temp, self.num_actions) * self.target_q, axis=1)
            
        target_value = self.rew_t_ph + (1. - self.done_mask_ph) * gamma * q_t_plus
        current_estimate = tf.reduce_sum(tf.one_hot(self.act_t_ph, self.num_actions) * self.q_network, axis=1)
        self.total_error = tf.reduce_mean(huber_loss(target_value - current_estimate))
        ######
    ...
    def step_env(self):
        ...
        # YOUR CODE HERE
        t_obs_idx = self.replay_buffer.store_frame(self.last_obs)

        if np.random.random() < self.exploration.value(self.t) \
                or not self.model_initialized \
                or not self.replay_buffer.can_sample(self.batch_size):
            action = self.env.action_space.sample()
        else:
            r_obs = self.replay_buffer.encode_recent_observation()[np.newaxis, ...]
            curr_q_eval = self.q_network.eval({self.obs_t_ph: r_obs}, session=self.session)
    ...
    def update_model(self):
        ...
        if (self.t > self.learning_starts and \
            ...
            if not self.model_initialized:
                initialize_interdependent_variables(self.session, tf.compat.v1.global_variables(), {
                    self.obs_t_ph: obs_t_ph,
                    self.obs_tp1_ph: obs_tp1_ph,
                })
    ...
    def log_progress(self):
        ...
        if self.t % self.log_every_n_steps == 0 and self.model_initialized:
            ...
            backup_data.write_log_file(step)

            self.mean_rews.append(self.mean_episode_reward)
            self.best_mean_rews.append(self.best_mean_episode_reward)
            self.steps.append(self.t)

            if self.t % 500000 == 0:
              plot(self.steps, self.mean_rews, self.best_mean_rews)
